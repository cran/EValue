...
# #' evalues.RG( beta = 1.4, se = .5, n = 100 )
# #' 
# #' # using p-value
# #' evalues.RG( pval = 0.03, n = 100 )
# 
# evalues.RG = function( beta=NA, se = NA, pval = NA, n = NA, true = 0 ) {
#   
...
#' @param true The true MD to which to shift the observed point estimate. Typically set to 1 to consider a null true effect. 
#' @export
#' @examples
...
evalues.HR = function( est, lo = NULL, hi = NULL, rare = NULL, true = 1 ) {
  ...
  # sanity checks
  if ( est < 0 ) stop("This outcome type cannot be negative")
  if ( is.null(rare) ) stop("For outcome type OR or HR, must specify whether outcome is rare")
...
evalues.RR = function( est, lo = NA, hi = NA, true = 1 ) {
  ...
  if ( !is.na(null.CI) & null.CI == TRUE ) warning("The null hypothesis cannot be rejected\n even without unmeasured confounding, so E-value for CI was set to 1.") 
  ...
  # if CI cross null, set its E-value to 1
  if ( !is.na(null.CI) & null.CI == TRUE ) E[ 2:3 ] = 1
...
#' 
#' Computes E-value for a single value of the risk ratio. 
...
#' @param true Null value of risk difference to which to shift the point estimate
...
evalues.RD = function(n11, n10, n01, n00,  
  ...
  if(p1 - p0 <= true) stop("RD is smaller than or equal to true value, so no unmeasured confounding needed to explain away effect.")
  ...
  if ( lowerCI <= true ) {
    ...
    warning("Lower CI of RD is smaller than or equal to RD^true.")
...
#' biasPlot(RR=3.9, xmax=20)

biasPlot = function(RR, xmax) {
...
#' Estimates and inference for sensitivity analyses for a meta-analysis
#'
...
#' Tables for sensitivity analyses for a meta-analysis
#'
...
#' Plots for sensitivity analyses for a meta-analysis
#'
...
#' Estimate proportion of population effect sizes above or below a threshold in a meta-analysis
#'
